...
public sealed class PolicyTests
{
    ...
    private static Policy Make(ImmutableArray<Namespace> namespaces) =>
        Assert.IsType<Result<Policy>.Success>(Policy.Create(namespaces)).Value;
    ...
    {
        var result = Policy.Create([Ns("doc", "viewer"), Ns("folder", "parent")]);

        var success = Assert.IsType<Result<Policy>.Success>(result);
    ...
    public void Create_EmptyNamespaces_ReturnsValidationFailure()
    {
        ...
        var failure = Assert.IsType<Result<Policy>.Failure>(result);
        var error = Assert.Single(failure.Errors);
        Assert.Equal(ErrorType.Validation, error.Type);
        Assert.Equal("policy.empty", error.Code);
    }

    [Fact]
    public void Create_DefaultArray_ReturnsValidationFailure()
    {
        var result = Policy.Create(default);
    ...
    {
        var result = Policy.Create([Ns("doc", "viewer"), Ns("folder"), Ns("doc", "editor")]);
    ...
    {
        var result = Policy.Create([Ns("doc"), Ns("folder"), Ns("doc"), Ns("folder")]);

        var failure = Assert.IsType<Result<Policy>.Failure>(result);
        Assert.Equal(2, failure.Errors.Length);
        Assert.All(failure.Errors, error => Assert.Equal("policy.duplicate_namespace", error.Code));
    ...
    public void Create_NamesDifferingOnlyByCase_AreDistinct()
    {
        ...
        var result = Policy.Create([Ns("doc"), Ns("Doc")]);

        _ = Assert.IsType<Result<Policy>.Success>(result);
    }
}
