...
summary: "Fact/schema drift is prevented at the Write service, not tolerated at the evaluator: fact writes validate against the current schema, domain writes that would abandon live facts are refused, and evaluation always reads a coherent snapshot pair. Removal becomes a two-step migration."
...
Facts and domains are separately writable artifacts that reference each other. Drift — a stored fact naming a namespace or relationship the schema no longer defines — has exactly two producers, and the Write service, as sole writer of both artifacts ([[four-service-split-by-load-profile]]), closes both:
...
2. **Domains can't abandon facts.** A domain write that removes a namespace or relationship is refused while live facts reference it. The check is a reverse existence query — do any live facts reference this name — at domain-write time, cold path. Removal becomes a two-step migration: migrate the facts, then land the removal. Whole-namespace and whole-domain deletion are the limiting case of the same ceremony.
...
- **Allow abandonment, keep condition 4 as an expected path** — domain agility over fact integrity: destructive domain pushes land in one step and evaluations meeting stranded facts fail as modeled errors. Lost: an authorization system values fact integrity more, and an expected-path error on the hot walk is an outage that looks like policy.
