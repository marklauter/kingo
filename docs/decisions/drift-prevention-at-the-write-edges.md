...
summary: "Fact/schema drift is prevented at the Write service, not tolerated at the evaluator: fact writes validate against the current schema, schema writes that would abandon live facts are refused, and evaluation always reads a coherent snapshot pair. Removal becomes a two-step migration."
...
- **Allow abandonment, keep condition 4 as an expected path** — schema agility over fact integrity: destructive schema pushes land in one step and evaluations meeting stranded facts fail as modeled errors. Lost: an authorization system values fact integrity more, and an expected-path error on the hot walk is an outage that looks like policy.
