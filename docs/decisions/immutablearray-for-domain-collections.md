...
Domain values in Kingo are read-optimized by construction. "Mutation" in this design means the Write service constructs a whole new `Namespace` value and persists it as the next version — nothing ever edits a schema definition in place. Downstream (Check, Expand, Read) only ever traverses. That is exactly `ImmutableArray`'s sweet spot; `ImmutableList`'s structural sharing would pay its overhead for update operations that never happen.
