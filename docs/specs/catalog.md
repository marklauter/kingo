...
The catalog is the space every name resolves in. A rewrite in one theory may reference a relation in another, and that reference resolves against the catalog, not the theory that wrote it. A [[fact]] may span theories the same way, each side qualified independently ([[facts]]).
