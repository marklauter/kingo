...
A theory projects to a YAML document with two keys. `theory:` is the theory's name; `namespaces:` maps each namespace name to its relationship list. The name travels inside the projection rather than arriving out of band, so the printer emits every part of the theory it was given and the document round-trips. A write carries a whole document and replaces the theory whole. Omit a relationship and the write removes it.
...
Every name a rewrite holds is a `⟨relationship name⟩`, evaluated against the resource in hand. Its character grammar — `⟨name-start⟩` through `⟨digit⟩` — also forms the `theory:` value and the namespace keys, the `⟨theory name⟩` and `⟨namespace name⟩` productions in [[identifiers]].
