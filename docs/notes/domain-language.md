...
| namespace               | Namespace    |
| relation tuple          | Fact         |
| namespace configuration | Spec (the owned set of namespaces an SDL document defines) |
...
- `Spec(Namespaces)` — the complete spec as a value: what a spec document (SDL, JSON) defines. **The config-side aggregate root** (2026-07-14; see aggregate table): a set of namespaces curated together, the unit of atomic config change. Never empty (`spec.empty`: the absence of namespaces is the absence of a spec — model that as not having one) and names are unique within it (`spec.duplicate_namespace`); both enforced by `Create`, the only construction path (ctor private, same rule as `Namespace`). No core `Parse` — adapters parse documents (`SdlDocument.Parse`) and project a `Spec` from the result. Store-wide checks stay with Write.
- `Relationship(Name, Rewrite)` — a named relationship and its rewrite, inside a `Namespace`. The name is free for the spec record because the stored fact is `Fact` — the relationship is the *predicate* of the triple, not the triple itself. Pre-reboot main called this `RelationshipSpec`; Pdl called it `Relation`.
...
Two aggregate roots, one per side of the system: config and data. Aggregates are consistency boundaries around **stored state** — the earlier four-root table (Namespace/Resource/Subject/Fact, then Spec/Resource/Subject/Fact) was grammar-driven ("every grammar element carries an aggregate boundary") and collapsed once the state test was applied: only `Spec` and `Fact` are ever stored. Each root has a unique, immutable **domain key** (never the storage hash key or a PK surrogate; dictionary-encoded ints are a storage-adapter concern that never leaks into the domain):

| Aggregate root | C# namespace          | Domain key |
|----------------|-----------------------|------------|
| `Spec`       | `Kingo.Schemas`      | `SpecPath` — the name, name-as-identity (2026-07-15, provisional: see open items). Same grammar as `NamespacePath` (`^[A-Za-z_][A-Za-z0-9_]*$`, lowercase-normalized) because it is the same kind of thing — authored vocabulary, unlike a client-minted `<resource-id>`. Replaced `Namespace` as the config-side root 2026-07-14. `Namespace` is now an entity within `Spec`, identity local (spec + `NamespacePath`); the name-is-the-identity reasoning still holds one level down: no rename, only a new namespace. (A separate ULID-style key for namespaces was considered and rejected: it buys admin identity but not rename-freedom, at the cost of Terraform-style illegible facts.) |
...
- **`Spec` aggregate follow-ups** (root swap executed 2026-07-14: `Kingo.Schemas` C# namespace, table above updated; `Spec` is a set of namespaces *curated together* — grouped by human intent, not required to reference each other; no referential-closure invariant, though a rewrite reference to a namespace outside the spec is dangling and checkable at `Create`). The domain key is **settled provisionally 2026-07-15**: `SpecPath`, name-as-identity — so no rename, only a new spec; `Spec.Create(name, namespaces)` carries it and SDL's `schema:` key spells it ([[domains]]). Worth revisiting once the reference question below lands: the ULID-style key was rejected for *namespaces* because a surrogate would make every fact illegible, but that argument only bites if the identifier appears in the fact grammar. If spec scope is resolved out-of-band, a surrogate key plus a mutable display name becomes available again — and admin rename-freedom matters more for a spec than for a namespace. Still open: how spec scope appears in references — **Spec picks up addressability** within an installation (two specs may both define `doc`), so either the fact grammar grows a spec segment or facts/resources carry `SpecPath` outside the text form. Spec versioning feeds [[storage-versioning-design]]. **Accounts stay out of the domain**: an account holds many specs administratively, but Kingo's multi-tenant mode is independent installations (a container or isolated AWS account per customer) — tenancy is deployment, never a domain key.
...
- [[domains]] — the authoring format for the spec model.
