...
The `#...` marker is tuple-grammar punctuation (Zanzibar §2.1), not an SDL-ism and not a relationship: it closes the resource-member production, so `folder:y#...` is the resource itself as a member — a `Fact.ResourceFact` (ruled 2026-07-21; [[resource-fact-case]]).
