...
| namespace               | Namespace    |
| relation tuple          | Fact         |
| namespace configuration | Domain (the owned set of namespaces an SDL document defines) |
...
**The language (2026-07-15):** the umbrella over Kingo's authored formats is the **Authorization Graph Language (AGL)**, with two sublanguages — **SDL**, the Schema Definition Language ([[domains]]), and **FML**, the Fact Mutation Language ([[graph-document-is-bulk-dml]]) — standing to AGL as DDL and DML stand to SQL. "Graph" is the unifying frame settled above: the schema defines what edges may exist, the graph is the edges that do, so both halves are *about the graph* the same way DDL and DML are both about data (SQL's own umbrella word doesn't literally cover `CREATE TABLE` either). The name is deliberately brandless and carries no `Kingo` prefix — SQL's "Structured" is a property, not a product name, and the paradigm outlives the implementation. It spends nothing the guardrail forbids: AGL names a language, not a type, and `Graph` stays a concept plus the `Kingo.Facts` assembly, never a domain value. It is also sized for what may come — Check is a reachability walk over the graph, so a query sublanguage would fit AGL without a rename, where an authoring-only name would not. Rejected: `KQL` (Kusto and Kibana are both live query languages), `KAL`/`KGL` (brand prefix), and naming the umbrella nothing at all (defensible — the two sublanguages share no grammar, only a frame — but the DDL/DML frame is load-bearing enough in these notes to deserve a word).
