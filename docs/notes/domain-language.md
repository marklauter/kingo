...
| `<tuple>`         | `Fact`                | the stored fact — a set-membership assertion, an RDF triple read set-first (settled 2026-07-15): the RDF-subject is the `SubjectSet`, the predicate is membership itself (∋), the RDF-object is the `Subject` — so Zanzibar's tuple order isn't backwards, and the param order mirrors the text form. `Fact(SubjectSet, Subject)` **structurally**, not `(Resource, Relationship, Subject)` (reshaped 2026-07-16): the grammar note above already read `<tuple>` as `<subjectset> '@' <subject>`, and the set-first triple reading names `SubjectSet` as one thing — the flattened three-part shape contradicted both, and made `SubjectSet` a derived property reconstructing on every access. The pair is exactly the question `Contains` asks ([[rewrite-interpreters]]). Named for what it is (an assertion); `Grant` was rejected (structural edges like `folder:a#parent@folder:b` grant nothing) and `Entitlement` names the *derived* output of Expand, not the stored datum. `Subject` over `principal`: subject is the standard term for the party an assertion is about (JWT `sub`, XACML); principals are authn-side and map onto a subject |
