...
- The fact **model** is unchanged — still (resource, relation, subject), the spec riding in on the namespace id, no new field. The fact **notation** changes: every namespace a fact names is spelled `<spec>/<namespace>`, a mandatory prefix, and a bare namespace is no longer legal to write. Free in the tuple, not free at the surface.
