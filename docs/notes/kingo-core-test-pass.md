...
2. Then: rebuild the PDL parser/serializer as `Kingo.Serialization.Pdl` against the new core types per [dissolve-kingo-pdl-under-hexagonal-layout](dissolve-kingo-pdl-under-hexagonal-layout.md).
3. Later, unblocked by neither: the rewrite interpreters (Check's boolean walk, Expand's tree) with tuple lookup as a port, per [four-service-split-by-load-profile](four-service-split-by-load-profile.md).
