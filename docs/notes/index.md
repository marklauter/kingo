...
- [[architecture]] — hexagonal with a DDD core: `Kingo` holds domain types; `Kingo.Documents` is the theory-document codec; `.Json`/`.Yaml` are value-type converter packs for future REST hosts. No ports project — the first genuine port family (storage, transport) gets its own.
...
- [[the-first-consumer-forges-the-domain]] — building the theory-document codec pressure-tested the core: the renames, the aggregate collapse, and the port dissolution all fell out of one real consumer; put one on a young domain early.
