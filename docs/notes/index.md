...
- [[graph-document-is-bulk-dml]] — **open**; proposal 2026-07-15: the fact-side document is a list of `create`/`touch`/`delete` operations in YAML section blocks, parsing to a `FactOperation` DU. Conflicts with `GraphParser` as stubbed, empties `GraphPrinter`, and vindicates the `Graph`-is-not-a-type guardrail.
