...
Three families, eight conditions. Every one is a `Result.Failure` — where no membership answer exists there is no `Decision` — and the Kleene rule decides whether a given evaluation surfaces or absorbs it.
...
5. **Tupleset member over-specified.** Traversal met a `SubjectSet` member carrying a real relationship — `folder:x#parent@folder:y#viewer` — where the exactly-specified shape is `folder:y#...` (`RelationshipIdentifier.Nothing`). Per the locked tupleset ruling in Settled: no override semantics, no silent skip.
