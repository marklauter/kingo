...
The Write API builds `Theory` values without touching `Kingo.Documents`; a parse-time guard leaves that path open. Guarding `Namespace.Create` covers every producer, and the theory-document parser inherits it.
...
- **Cycle errors carry the full cycle path**, so a defective theory is diagnosable from the user's chair without re-deriving the graph.
