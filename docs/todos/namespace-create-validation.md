...
The Write API builds `Domain` values without touching `Kingo.Sdl`; a parse-time guard leaves that path open. Guarding `Namespace.Create` covers every producer, and SDL inherits it.
...
- `src/Kingo.Sdl/RewriteExpressionParser.cs` — constructs through the `Create` factories, `.Map` → `.Bind` at the fallible operators; the SDL example in [[domains]] gained the `parent` definitions the gate now requires.
- Pinned by `tests/Kingo.Domains.Tests/NamespaceTests.cs`, `SubjectSetRewriteTests.cs`, and the SDL suites (`DomainParseTests` carries parse-path `namespace.dangling_reference` / `namespace.rewrite_cycle` rows).
