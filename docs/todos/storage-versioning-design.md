...
- **Optimistic concurrency on writes** — the Write host is the sole writer and appends the changelog (the zookie source). The [[choosing-the-storage-substrate]] mapping table leaned on `[DynamoDBVersion]` for CAS, but the key/value store style settled 2026-07-14 drops the ORM — version-checked writes are now hand-rolled `ConditionExpression`s in the storage adapter.
...
- [[choosing-the-storage-substrate]] — the substrate the scheme must be expressed in
