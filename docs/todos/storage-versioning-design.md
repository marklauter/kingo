...
- Design session when storage work begins (queued behind the SDL adapter and converters per [[index]]): encode the settled shapes — the interval-stamp attributes on fact items, the domain changelog's item form — define `Kookie` and `DomainVersion` encodings as functions of them, and specify the conditional-write patterns the Write adapter uses, including the domain-write reverse existence query and the domain-at-K lookup.
