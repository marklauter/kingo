...
- **SDL round-trips** (`tests/Kingo.Sdl.Tests`): generate domains, assert print→parse identity — the existing `SpecRoundTripTests` enumerate cases by hand; a generator walks the space between them.
