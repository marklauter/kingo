...
- F18 inverts ([[rewrite-interpreters-findings]]): `Contains(folder:y#...@user:anne)` is unrepresentable — `...` fails `RelationshipIdentifier.Parse`, so the question dies at the parse edge and never reaches condition 2's schema lookup. The named test pins the parse refusal instead.
...
- [x] [[rewrite-interpreters-findings]]: F18 annotated as inverted (2026-07-21).
