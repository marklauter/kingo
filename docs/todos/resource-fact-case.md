...
- Dry-run finding F18 inverts: `Contains(folder:y#...@user:anne)` is unrepresentable — `...` fails `RelationshipIdentifier.Parse`, so the question dies at the parse edge and never reaches condition 2's theory lookup. The named test pins the parse refusal instead.
...
- [x] [[ubiquitous-language]]: `<subject>` production gains `<resource> '#...'`; the `<relationship>` terminal drops `...`; the Fact mapping row carries the third case (2026-07-21).
