...
2.1. **`docs/decisions/schema-dissolves-into-administration.md`** — the decision is reversed, but its *shared-groups argument* is load-bearing and survived the reversal: an org-wide `group:eng#member` has no owner that can hold it usefully, so every wall would force a membership copy and resurrect group-sync drift. That's the argument for why a theory is not a referential wall. [[specs/catalog]] asserts this ("a rewrite in one theory may reference a relation in another") but doesn't argue it. Move the argument into [[specs/catalog]] or a decision record; drop the file.
