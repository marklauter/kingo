...
/// <summary>
/// Discriminated result of a domain operation — either a successful <typeparamref name="T"/> value (<see cref="Result{T}.Success"/>) or a non-empty collection
/// of <see cref="Error"/>s (<see cref="Result{T}.Failure"/>). Consume via <see cref="Match"/>, transform via <see cref="Map"/>, chain via <see cref="Bind"/> or
/// <see cref="BindAsync"/>; <see cref="Select"/> and the
/// <see cref="SelectMany{TIntermediate, TResult}(Func{T, Result{TIntermediate}}, Func{T, TIntermediate, TResult})"/> shapes are LINQ-named aliases of the same
/// operations. Combine independent results via <see cref="Result.Apply{T, TResult}(Result{System.Func{T, TResult}}, Result{T})"/> (applicative — accumulates
/// errors on the failure path) or the Unit-keyed <c>Result.Apply</c> overload (effect sequencing). The inheritance hierarchy is closed —
/// <see cref="Result{T}.Success"/> and <see cref="Result{T}.Failure"/> are the only inhabitants — and dispatch is virtual: each inhabitant implements the
/// combinators, so exhaustiveness is a compile-time fact (a new inhabitant cannot build without them) rather than a runtime switch.
/// </summary>
...
public abstract record Result<T>
{
    ...
    /// through the <c>Result.Failure</c> factories, which enforce it. Equality is structural over <see cref="Errors"/> (element-wise, order-sensitive) —
    /// <see cref="ImmutableArray{T}"/>'s default record equality would otherwise compare the underlying array reference.
    /// </summary>
    ...
    /// <summary>
    /// Async monadic bind: chain a result-returning async function after a successful result; short-circuit on failure. Cancellation is the responsibility of
    /// <paramref name="fn"/> — no <see cref="System.Threading.CancellationToken"/> is threaded through the API; callers needing cancellation pass a lambda that
    ...
    /// <summary>
    /// LINQ-named alias of <see cref="Map"/>: transform the success value with <paramref name="selector"/>; pass the failure through unchanged.
    /// </summary>
    ...
    /// <summary>
    /// Monadic bind with projection — the shape the compiler binds for chained <c>from</c> clauses in query syntax. Equivalent to
    /// <c>Bind(x =&gt; selector(x).Map(y =&gt; resultSelector(x, y)))</c>: short-circuits on the first failure, so errors do not accumulate across clauses
...
public static class Result
{
    ...
    /// <summary>
    /// Construct a failed result from a single <see cref="Error"/>. The success type <typeparamref name="T"/> must be supplied explicitly — it cannot be
    /// inferred from <paramref name="error"/>.
    /// </summary>
    public static Result<T> Failure<T>(Error error) => new Result<T>.Failure([error]);

    /// <summary>
    /// Construct a failed result from one or more <see cref="Error"/>s. Throws <see cref="ArgumentException"/> when <paramref name="errors"/> is empty — a
    /// failure must carry at least one error. <c>params ReadOnlySpan</c> keeps varargs argument lists off the heap; collection expressions and pre-built
    /// <see cref="ImmutableArray{T}"/>s bind to the <see cref="ImmutableArray{T}"/> overload instead (see its
    ...
    /// <summary>
    /// Construct a failed result from a pre-built <see cref="ImmutableArray{T}"/> of <see cref="Error"/>s. Cheapest factory — the array is stored directly with
    /// no copy, and collection expressions (<c>Failure&lt;T&gt;([a, b])</c>) build it directly. Throws <see cref="ArgumentException"/> when
    /// <paramref name="errors"/> is default or empty. The priority attribute breaks the tie with the <see cref="ReadOnlySpan{T}"/> overload that
    ...
    /// <summary>
    /// Construct a failed result from a list of <see cref="Error"/>s. Throws <see cref="ArgumentException"/> when <paramref name="errors"/> is empty — a
    /// failure must carry at least one error.
    /// </summary>
    ...
    /// <summary>
    /// Lift a boolean check into a <see cref="Result{T}"/> of <see cref="Unit"/>: success when <paramref name="condition"/> holds, otherwise a failure carrying
    /// <paramref name="error"/>. The entry point for applicative accumulation — lift each independent check, then combine with the variadic
    ...
    /// <summary>
    /// Applicative application: feed a <see cref="Result{T}"/>-wrapped argument to a <see cref="Result{T}"/>-wrapped function. Multi-arity handled by currying
    /// — apply repeatedly to consume each argument. When both <paramref name="resultFn"/> and <paramref name="resultArg"/> fail, their errors are accumulated
    /// (function errors first, then argument errors); this is the Validation-applicative behaviour rather than fail-fast.
    /// </summary>
    ...
    /// <summary>
    /// Variadic effect sequencing: combine any number of <see cref="Result{T}"/>s of <see cref="Unit"/>. Succeeds when every input succeeds (and when
    /// <paramref name="results"/> is empty — the identity element); otherwise returns a <see cref="Result{T}.Failure"/> whose errors are accumulated across all
    /// failed inputs in input order. <c>params ReadOnlySpan</c> keeps the argument list off the heap at every arity, and the single-failure path returns the
    /// failing input unchanged rather than copying its errors.
    /// </summary>
