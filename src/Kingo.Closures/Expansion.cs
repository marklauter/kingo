...
/// Contains's short-circuiting boolean verdict (<see cref="Decision"/>). Expected to carry the <see cref="Kingo.Graphs.SubjectSet"/> asked about, the
/// materialized rewrite tree (referenced subject sets stay leaves — single-level), the snapshot pin, the spec version, and the wall timestamp — the
