...
internal sealed class SqliteDocumentWriter<D>(IDbContext context)
{
    ...
    private static class Header
    {
        ...
        private readonly record struct UpdateParam<HK, RK>(HK HashKey, RK? RangeKey, Revision OldVersion, Revision NewVersion);
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Task<int> UpdateAsync<HK, RK>(HK hashKey, RK? rangeKey, Revision oldVersion, Revision newVersion, DbConnection db, DbTransaction tx) =>
            UpdateStatement
            .Match(
                Some: statement => db.ExecuteAsync(statement, new UpdateParam<HK, RK>(hashKey, rangeKey, oldVersion, newVersion), tx),
        ...
        public static Task<Revision?> ReadRevisionAsync(D document, DbConnection db, DbTransaction tx) =>
            RevisionStatement
            .Match(
                Some: statement => db.QuerySingleOrDefaultAsync<Revision?>(statement, document, tx),
                None: () => Task.FromResult((Revision?)null));
    }
