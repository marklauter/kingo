...
/// with structural equality. Parse-agnostic and storable. An entity within the <see cref="Policy"/> aggregate,
/// not a root: its identity is local — <see cref="Name"/> unique (case-insensitive) within its policy, immutable
...
public sealed record Namespace
{
    ...
    /// <see cref="ErrorType.Validation"/> error per duplicated name in first-occurrence order. The policy model has no core <c>Parse</c> — its text forms live
