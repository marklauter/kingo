...
/// <summary>
/// Parses the rewrite-expression mini-language embedded in PDL relationship values — e.g. <c>(this | editor | (parent, viewer)) ! banned</c> — into the core
/// <c>SubjectSetRewrite</c> algebra. Grammar and precedence: docs/notes/pdl-yaml.md (<c>!</c> binds tightest; <c>&amp;</c> and <c>|</c> share precedence,
...
internal static class RewriteExpressionParser
{
    public static Result<SubjectSetRewrite> Parse(string expression)
    {
        ...
        if (!tokens.HasValue)
            return Result.Failure<SubjectSetRewrite>(Error.Validation("pdl.rewrite", $"invalid rewrite expression '{expression}': {tokens}"));

        var parsed = Expression.AtEnd().TryParse(tokens.Value);
        return parsed.HasValue
            ? Transform(parsed.Value)
            : Result.Failure<SubjectSetRewrite>(Error.Validation("pdl.rewrite", $"invalid rewrite expression '{expression}': {parsed}"));
    }
