...
/// <summary>
/// Renders a <c>SubjectSetRewrite</c> tree as rewrite-expression text (grammar: docs/notes/pdl-yaml.md). Parenthesization is decided by grammar position so the
...
internal static class RewriteExpressionRenderer
{
    ...
    private static string RenderIdentifier(RelationshipIdentifier relationship) =>
        IsReserved(relationship)
            ? throw new ArgumentException($"relationship '{relationship}' cannot be referenced in a PDL rewrite expression: 'this' and '{RelationshipIdentifier.Nothing}' are reserved by the grammar")
            : relationship.Value;
